...
class MyPageFragment : Fragment() {
        ...
        if (user != null) {
            ...
            fetchUserUid(uid) { result ->
                if (result != null) {
                    ...
                    Log.d("MyPageFragment", "Failed to fetch result or no data found.")
                }
            }
        } else {
    ...
    private fun fetchUserUid(uid: String, callback: (String?) -> Unit) {
        val diagnosticRef = FirebaseFirestore.getInstance().collection("User").document(uid).collection("results")
        diagnosticRef.orderBy("date", Query.Direction.DESCENDING).limit(1)
            ...
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    ...
                    val result = document.getString("results")
                    callback(result)
                }
            }
    ...
    private fun updateProductDetails(res: String) {
        ...
        toneRef.get().addOnSuccessListener { document ->
            if (document.exists()) {
                extractAndDisplayProductDetails(document)
            } else {
    ...
    private fun extractAndDisplayProductDetails(document: DocumentSnapshot) {
        val productDescriptions = document.get("제품설명") as? List<Map<String, Any>> ?: listOf()
        displayProductDetails(productDescriptions, "베이스", baseProductTextViews)
        displayProductDetails(productDescriptions, "립", lipProductTextViews)
        displayProductDetails(productDescriptions, "아이", eyeProductTextViews)
    }

    private fun displayProductDetails(descriptions: List<Map<String, Any>>, category: String, views: List<TextView>) {
        val categoryMap = descriptions.find { it.containsKey(category) }?.get(category) as? Map<String, Any> ?: mapOf()
        categoryMap.let { products ->
            views.forEachIndexed { index, textView ->
                val productKey = "${category}제품${index + 1}"
    ...
    private fun setupProductImageViews(view: View) {
